...
export const handler = async (event) => {
  try {
    ...
    if (!gid) return { statusCode: 400, body: "Missing gid" };
    ...
    // Delete group doc and optionally subcollections (careful)
    await gRef.delete();
    // Note: Firestore doesn't cascade delete subcollections automatically. For production, use a background function or
    // store a 'deleted' flag and run a clean up script that wipes subcollections.
